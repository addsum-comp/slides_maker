...
最低成本的开场是零 prompt。在 Claude Code 里输入：

```text
/slide-maker
```

访谈立即开始：一个带标签页的问卷（主题、模板、用途受众、风格语言），每题都带现成选项，方向键切换、回车选择。主题那一页甚至会根据你近期的项目猜出几个候选，猜不中就直接输入你的。全部答完点 Submit，它开工。

手里有材料的话，一句话开场也行，它会跳过主题、从剩下的问起：
...
两个工具都完整支持，体验上更推荐 Claude Code：访谈是带选项的表单，方向键加回车点选就走完。Codex 没有斜杠命令，说一句「用 slide-maker 按 paper.pdf 做 PPT」同样触发，只是问题变成文字追问，要逐条打字回答。两边都是短句就够，「你定」永远是合法答案，回「先给我几个风格方向」还能拿到渲染好的风格候选挑一个。
