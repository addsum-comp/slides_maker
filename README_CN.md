...
- **content-planner（内容规划）**：牵头的策划兼设计。它负责把材料真正通读一遍，决定每页讲什么、哪张图放哪、deck 的节奏、动画到底值不值得用。整份 deck 出自同一个脑子，所以是一条连贯的论证，而不是一堆散页。
- **critic（评审）**：一个没参与搭建的独立评审。它对着**渲染出来的**页面按演讲目的挑毛病：版式挤压、对比度不足、数字和原文对不上，统统打回重修。正因为它对草稿没有立场，才够诚实。
- **arbiter（仲裁）**：重要场合的 deck 会加这道二次复核：动手改之前，先确认评审挑的毛病是真毛病，修的是真问题，而不是噪声。
- **asset-prep（素材准备）**：构建期的执行工。你确认结构稿之后，它**并行**把计划里的素材（裁图、公式图、示意图）做出来，不做任何设计决策，所以再大的 deck 也出得快。
