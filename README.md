...
- `lint_deck.py` — deterministic layout lint on a built `.pptx`: flags off-slide overflow, **text overflowing its card**, **uneven card heights in a row**, solid block/image overlaps (ignoring intentional layering), footer collisions, **orphaned punctuation / widow** (a lone 。/，or single glyph on the last line — 避头尾), **CJK text with no EA font** (the kinsoku root cause), whole-page-image/editability, and orphan/empty slides. Run after rendering, before the critic.
