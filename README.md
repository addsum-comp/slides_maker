...
- `lint_deck.py` — deterministic layout lint on a built `.pptx`: flags off-slide overflow, solid block/image overlaps (ignoring intentional layering), and footer collisions. Run after rendering, before the critic.
